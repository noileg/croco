...
このファイルの中身をそのまま `クロコ関係/CLAUDE.md` に置くと、
以後のセッションで自動的に読み込まれる（各セッションの冒頭で毎回コンテキストに載る）。

**狙いはトークン消費の削減。** 効くのは「調べ直し・読み直しをさせない」こと。
このセッションで消費が大きかったのは、仕様書の通読、APIの仕様調査、
有効な値を総当たりで探る作業、環境固有のトラブル切り分けだった。
いずれも**一度分かれば二度と調べる必要がない**ので、結論だけをここに固定する。

逆に、ここに書きすぎると毎回のコンテキストを圧迫して本末転倒になる。
「調べ直しを防ぐ事実」と「読むべき場所への案内」に絞ること。
